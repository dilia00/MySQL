...
FROM cats c -- , левая таблица
JOIN shops s --  правая таблица
ON s.id = c.shops_id; 
...
RIGHT JOIN  shops s 
ON s.id = c.shops_id 
WHERE s.id != (SELECT shops_id FROM cats WHERE `name` = "Murzik") 
AND s.id != (SELECT shops_id FROM cats WHERE `name` = "Zuza");
...
SELECT o.ord_datetime, a.an_price, a.an_name
FROM Orders o 
JOIN analysis a
ON o.ord_an = a.an_id
