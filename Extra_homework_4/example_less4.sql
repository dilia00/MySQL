DROP DATABASE IF EXISTS ex_lesson_4;
CREATE DATABASE ex_lesson_4;
USE ex_lesson_4;
...
SELECT 
    ...
    (SELECT hometown FROM profiles WHERE user_id = users.id) AS "Город рождения",
    (SELECT filename FROM media WHERE id = (SELECT photo_id 
    FROM profiles WHERE user_id = users.id)) AS "Аватарка"
FROM users;
...
-- выбрать фотографии (filename) пользователя с email: arlo50@example.org . ID типа медиа, 
...
-- (используя JOIN)
SELECT m.user_id, m.filename, t.name_type
FROM media m
JOIN media_types t
ON t.id = m.media_type_id 
ORDER BY user_id
